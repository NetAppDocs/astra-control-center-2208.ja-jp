...
Astra Control Centerのインストール中に、を指定した場合 `enrolled: true` の場合 `autoSupport` Astra Control Center Custom Resource Definition（CRD）ファイル (`astra_control_center_min.yaml`）を指定すると、日次サポートバンドルがに自動的にアップロードされます https://mysupport.netapp.com/site/["NetApp Support Site"^]。
